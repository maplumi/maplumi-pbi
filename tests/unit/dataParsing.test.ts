...
describe("data parsing helpers", () => {
    ...
    test("parseChoroplethCategorical extracts roles", () => {
        const categorical: any = {
            categories: [{ source: { roles: { AdminPCodeNameID: true } }, values: ["P1", "P2"] }],
            values: [{ source: { roles: { Color: true }, queryName: "m" }, values: [1, 2] }],
        };
    ...
    test("parseCircleCategorical extracts coords and sizes", () => {
        const categorical: any = {
            categories: [
                { source: { roles: { Longitude: true } }, values: [10, 20] },
                { source: { roles: { Latitude: true } }, values: [30, 40] },
            ],
            values: [
                { source: { roles: { Size: true }, queryName: "s1" }, values: [5, 10] },
                { source: { roles: { Size: true }, queryName: "s2" }, values: [2, 3] },
            ],
        };
...
describe("circle math helpers", () => {
    ...
    test("calculateCircleScale handles empty", () => {
        const res = calculateCircleScale([], circleOptions);
        expect(res.circleScale).toBe(1);
    });
    test("applyScaling clamps and scales", () => {
        const { minCircleSizeValue, maxCircleSizeValue, circleScale } = calculateCircleScale([1, 2, 3, 4, 100], circleOptions);
        const r = applyScaling(3, minCircleSizeValue, maxCircleSizeValue, circleScale, circleOptions, [1, 2, 3, 4, 100]);
        expect(r).toBeGreaterThan(0);
    });
    test("findClosestValue returns nearest", () => {
        const v = findClosestValue([1, 5, 10], 6);
        expect(v).toBe(5);
    });
});
