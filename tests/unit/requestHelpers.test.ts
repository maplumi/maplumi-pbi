...
describe('requestHelpers', () => {
  ...
  describe('hasOpenRedirect', () => {
    it('flags cross-origin absolute redirects', () => {
      expect(
        hasOpenRedirect('https://example.com/a?redirect=https://evil.com/x')
      ).toBe(true);

      expect(
        hasOpenRedirect('https://example.com/a?url=//evil.com/y')
      ).toBe(true);
    });

    it('flags javascript/data redirect values', () => {
      expect(
        hasOpenRedirect('https://example.com/a?next=javascript:alert(1)')
      ).toBe(true);
      expect(
        hasOpenRedirect('https://example.com/a?return=data:text/html;base64,AAAA')
      ).toBe(true);
    });
    ...
    it('allows safe relative redirects to same origin', () => {
      expect(
        hasOpenRedirect('https://example.com/a?return=/local/page')
      ).toBe(false);
      expect(
        hasOpenRedirect('https://example.com/a?redirect=%2Finternal%2Fhome')
      ).toBe(false);
    });
  });
  ...
  describe('fetchWithTimeout', () => {
    ...
    it('resolves when fetch succeeds before timeout', async () => {
      ...
      // Advance time a bit but not beyond the timeout
      jest.advanceTimersByTime(1000);
      await expect(p).resolves.toBe(mockResponse);
    });

    it('rejects when the request is aborted due to timeout', async () => {
      // A fetch mock that rejects when the provided signal aborts
      (global.fetch as jest.Mock).mockImplementation((_: string, init?: RequestInit) => {
